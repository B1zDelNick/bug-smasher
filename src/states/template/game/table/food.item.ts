...
export class FoodItem {
    ...
    private eatedSprite: Phaser.Sprite = null;
    private XX: number = 0;
    private YY: number = 0;
    ...
    constructor(parent: Phaser.Group, type: FoodType, x: number, y: number) {
        ...
        this.eatedSprite = this.game.add.sprite(0, 0,
            ImageUtils.getAtlasClass('AtlasesFood').getName(),
            ImageUtils.getAtlasClass('AtlasesFood').Frames[type + 'R'],
            this.container
        );
        GuiUtils.centrize(this.eatedSprite);
        this.eatedSprite.position.setTo(x, y);
        this.eatedSprite.alpha = 0;
    }

    public eat() {
        this.eated = true;
        TweenUtils.fadeOut(this.mainSprite, 250);
        TweenUtils.fadeIn(this.eatedSprite, 250, 0, () => {
            TweenUtils.fadeOut(this.eatedSprite, 500, 500);
        }, this);
    }
    public hide() {
        TweenUtils.fadeOut(this.mainSprite, 500);
    }
    ...
    public removePoison() {
        ...
        TweenUtils.moveInOut(this.poisonedSprite, this.XX, this.YY, 750, 0, () => {
            ...
            TweenUtils.fadeIn(this.mainSprite, 250, 0, () => {
    ...
    public update() {
        this.timeLeft--;
        if (0 >= this.timeLeft && !this.animating) {
            this.removePoison();
        }
    }
    ...
    public getPoisonPosition(): Phaser.Point {
        return this.poisonedSprite.position;
    }
    ...
    public dispose() {
        ...
        this.poisonedSprite.destroy(true);
        this.eatedSprite.destroy(true);
